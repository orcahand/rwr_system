...
def get_cropped_and_collor_maps(image, camera_id, color_detected = None ,output_dir=None):
    ...
    if camera_id == "wrist":
        ...
        mask = np.ones_like(image.shape, dtype=np.uint8) * 255
    ...
    if color_detected:
        ...
        gray_image_masked = np.copy(gray_image_bgr)
        gray_image_masked[condition_mask,:] = (255,0,0)

        return image_masked, masks_combined, gray_image_masked
